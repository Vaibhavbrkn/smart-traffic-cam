...
flags.DEFINE_string('annot', 'new', 'annotation path')
flags.DEFINE_string('input', 'input', 'input path')
...
def main(_argv):
    ...
    imagePaths = list(paths.list_images(FLAGS.input))
    # loop through images in list and run Yolov4 model on each
    for count, image_path in enumerate(imagePaths):
        file_name, file_extension = os.path.splitext(image_path)
        if file_extension != '.jpg':
            continue
        p = os.path.sep.join(
            [FLAGS.annot, "{}.png".format(str(vaibhav).zfill(6))])
        vaibhav += 1
        ...
        if FLAGS.count:
            ...
            image = utils.draw_bbox(original_image, pred_bbox, p, FLAGS.info, counted_classes,
                                    allowed_classes=allowed_classes, read_plate=FLAGS.plate)
        else:
            image = utils.draw_bbox(original_image, pred_bbox, p, FLAGS.info,
                                    allowed_classes=allowed_classes, read_plate=FLAGS.plate)
