...
import pytesseract
from license.core.config import cfg
import re
import requests
...
def minor_load_config(tiny, model):
    if tiny:
        STRIDES = np.array(cfg.YOLO.STRIDES_TINY)
...
def draw_bbox(image, bboxes, license_list, ob, info=False, counted_classes=None, show_label=True, allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), read_plate=True):
        ...
        if class_name not in allowed_classes:
            ...
            if read_plate:
                ...
                if plate_number not in license_list:
                    ...
                    url = "http://localhost:6000/{}".format(plate_number)
                    ...
                    if send_mail:
                        subject = "Summon for vehicle number {}".format(
                            plate_number)
                        body = f"Hi {head} {js['name']} Due to absence for required document for your vehicle number {plate_number}. you are fined with RS {fine}.\n kindly submit required amount to department of transport."

                        receivers = [js['mail']]

                        message = "Subject:{}\n\n{}".format(subject, body)

                        ob.sendmail("smart.traffic.cam@gmail.com",
                                    receivers, message)
